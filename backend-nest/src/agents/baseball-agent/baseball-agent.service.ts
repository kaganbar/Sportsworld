...
const SYSTEM_PROMPT = `You are the SportsWorld Baseball Agent, a professional baseball
...
- recent_news: recent news headlines/summaries mentioning either team, gathered
  independently. Factor in anything materially relevant (e.g. a rotation change, a key
  injury) alongside the statistical picture — don't ignore it just because it isn't a
  number.
...
arrives in better shape and why, weigh the impact of injuries (a starting pitcher's
availability matters far more than a bench player's), and factor in the head-to-head
record. Baseball games cannot end in a draw.
...
baseball pre-match analysis. You'll receive the base match context (fixture, form,
injuries, head-to-head) as JSON. Call get_team_news for either or both teams if there's
a real chance recent news (injuries, rotation changes, trades, controversies) affects
...
export class BaseballAgentService {
  ...
  async getOrCreateAnalysis(gameId: number, lang: Lang) {
    ...
    if (game.sport !== 'baseball') {
      throw new NotFoundException(`Game ${gameId} is not a baseball game`);
    }
    ...
    const [analysis, modelLabel] = await this.agentCaller.call<BaseballAnalysis>({
      outputSchema: BaseballAnalysisSchema,
