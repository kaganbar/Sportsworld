...
import { VolleyballAnalysisSchema, VolleyballAnalysis } from './volleyball-agent.schema';
...
const SYSTEM_PROMPT = `You are the SportsWorld Volleyball Agent, a professional volleyball
...
- recent_news: recent news headlines/summaries mentioning either team, gathered
  independently. Factor in anything materially relevant (e.g. a key hitter's injury, a
  lineup change) alongside the statistical picture — don't ignore it just because it
  isn't a number.
...
head-to-head record. Volleyball matches cannot end in a draw.
...
volleyball pre-match analysis. You'll receive the base match context (fixture, form,
injuries, head-to-head) as JSON. Call get_team_news for either or both teams if there's
a real chance recent news (injuries, lineup changes, controversies) affects this match
beyond what's already in the base context — skip it if the base context already looks
complete and current. Call get_independent_prediction if you want a statistical
cross-check to compare against later. Call whichever tools are useful, or none if the
base context is already sufficient — this is a lightweight enrichment step, not the
analysis itself. You do not need to write a final summary; a brief acknowledgment is
fine once you're done gathering.`;
...
@Injectable()
export class VolleyballAgentService {
  private readonly logger = new Logger(VolleyballAgentService.name);
   ...
   * captured tool results) — this mirrors it exactly, adapted to volleyball
   * teams/games. */
  private async enrichContext(gameId: number, lang: Lang, homeTeam: string, awayTeam: string, baseContext: unknown) {
    ...
    const tools = [
      ...
      betaZodTool({
        ...
        run: async () => {
          try {
            const prediction = await this.predictionAgent.getOrCreatePrediction('volleyball', gameId, lang);
            predictionResult = {
    ...
    try {
      ...
      this.logger.warn(`Volleyball Agent enrichment step failed for game ${gameId}, continuing without it: ${(err as Error).message}`);
    }
  ...
  private mockAnalysis(context: any, lang: Lang): VolleyballAnalysis {
  ...
  async getOrCreateAnalysis(gameId: number, lang: Lang) {
    ...
    // a different sport's game id hitting this endpoint would otherwise
    // short-circuit straight to that game's analysis instead of 404ing.
    const game = await this.games.findGameOrThrow(gameId);
    if (game.sport !== 'volleyball') {
      throw new NotFoundException(`Game ${gameId} is not a volleyball game`);
    }
    ...
    this.logger.log(`Requesting volleyball analysis for game ${gameId} (lang=${lang})`);
    const [analysis, modelLabel] = await this.agentCaller.call<VolleyballAnalysis>({
      outputSchema: VolleyballAnalysisSchema,
