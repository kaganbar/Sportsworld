...
export class BasketballAgentService {
   ...
   * captured tool results) — this mirrors it exactly, adapted to basketball
   * teams/games. */
  private async enrichContext(gameId: number, lang: Lang, homeTeam: string, awayTeam: string, baseContext: unknown) {
    ...
    const tools = [
      ...
      betaZodTool({
        ...
        run: async () => {
          try {
            const prediction = await this.predictionAgent.getOrCreatePrediction('basketball', gameId, lang);
            predictionResult = {
    ...
    try {
      ...
      this.logger.warn(`Basketball Agent enrichment step failed for game ${gameId}, continuing without it: ${(err as Error).message}`);
    }
