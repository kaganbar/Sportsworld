...
function makeService(deps: ReturnType<typeof makeDeps>, config: ConfigService, redis: RedisService) {
  return new BasketballAgentService(
    deps.prisma,
    deps.games,
    deps.stats,
    deps.translations,
...
describe('BasketballAgentService enrichment (Phase A)', () => {
  beforeEach(() => {
    mockParse.mockReset();
  ...
  it('tool results captured via closures appear in Phase B context', async () => {
    mockToolRunner.mockImplementation(async (params: any) => {
      ...
      await newsTool.run({ team_name: 'Lakers' });
    ...
    expect(deps.newsAgent.getNewsForSubject).toHaveBeenCalledWith('Lakers', 'en');
    expect(deps.predictionAgent.getOrCreatePrediction).toHaveBeenCalledWith('basketball', 1, 'en');
    const context = (deps.agentCaller.call as jest.Mock).mock.calls[0][0].context;
    expect(context.recent_news).toEqual({ Lakers: [{ headline: 'h', summary: 's', updated_at: '2026-01-01' }] });
  ...
  it('throws NotFoundException for a game that is not basketball', async () => {
    const deps = makeDeps();
    (deps.games.findGameOrThrow as jest.Mock).mockResolvedValue({ ...fakeGame, sport: 'football' });
    const service = makeService(deps, makeConfig({}), makeRedis(true));

    await expect(service.getOrCreateAnalysis(1, 'en')).rejects.toThrow('Game 1 is not a basketball game');
  });
});
