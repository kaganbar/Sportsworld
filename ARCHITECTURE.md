...
- ~~**Deployment target**~~ — **Resolved in Phase 11**: Vercel
  (`frontend-next/`) + Railway (`backend-nest/` + managed Postgres +
  managed Redis), confirmed via AskUserQuestion. See `DEPLOYMENT.md` for
  the full runbook.
...
| 6. Sports APIs | **Done.** Live scores: 365scores.com scraper. News: NewsAPI.org `top-headlines?category=sports`, hourly. Transfers: Transfermarkt's rumour-mill page (`/geruechte/aktuellegeruechte/statistik`), hourly, real rumors (player, clubs, competition, Transfermarkt's own community-probability %) into `TransferReport`/`ContentSource`, served at `GET /api/transfers`. Both News and Transfers have a light real frontend (raw lists, no AI summarization/dedup/story-grouping or our own probability estimate yet — that's Phase 8) |
| 7. Football Agent | Done |
| 8. Other AI Agents | **Done.** 5 new agent modules + Master Agent, all under `backend-nest/src/agents/`. General Sports Agent: no DB context, free-text Q&A only (confirmed scope — no real baseball/volleyball fixture data exists). Transfer Agent: dedups `TransferReport`s into `TransferStory` groups, estimates probability + writes `aiSummary`, hourly scheduler. Statistics Agent: real-data-only form/H2H digest, no fabricated advanced metrics. Prediction Agent: confirmed scope is a *general cross-sport synthesis tool* — independently re-predicts any sport's matchup (including football/basketball/tennis) rather than only covering sports without a dedicated agent. News Agent: heuristic title-keyword clustering + Claude summarization, hourly scheduler. Master Agent: the first tool-use loop in this codebase (`client.beta.messages.toolRunner` + `betaZodTool`, 8 tools wrapping the other agents' services via direct DI — not HTTP —, `max_iterations` round cap, `MasterReport` cache keyed on a query hash). Not wired into any frontend page yet. |
| 9. Realtime features | **Done.** Redis wired into `backend-nest` for all 3 roles from Phase 1's design: (1) WebSocket pub/sub — `LiveGateway` now subscribes to a shared `game-ticks` Redis channel instead of a local `EventEmitter2` event, and both tick producers (`NormalizeService`, `SimulatedTickerService`) publish to it instead of emitting locally, so a tick from any backend instance reaches clients connected to any other instance (verified against a real live game end-to-end). (2) Read-through cache — `GamesService.gamesToday` and `TennisService.matchesToday`, 20s TTL (safe since the frontend never re-polls these for live changes, only merges WS ticks on top of the initial list). (3) Rate limiting — a shared fixed-window counter (`anthropic-live-calls`, 20/60s) gates every real (non-mock, cache-miss) Claude call, inside `AgentCallerService.call()` and Master Agent's own top-level `toolRunner` call; every agent controller maps a trip to 429. |
| 10. Testing | **Unit tests done** (confirmed scope via AskUserQuestion: unit tests in both apps, no real test DB, no e2e, no CI — those are natural next increments). `backend-nest`: Jest (`npm test`), 6 spec files / 31 tests — pure logic (`probability-normalizer`, the 3 `scores365` parsers) and mocked-dependency service tests (`AgentCallerService`'s full error-path matrix, `RedisService.checkRateLimit`'s fixed-window logic). `frontend-next`: Vitest + Testing Library (`npm test`), 4 test files / 16 tests — `lib/utils.ts`, `lib/api.ts` (fetch-mocked), `AiAnalysisPanel` (rendered with an injected fetcher), `useLiveGame` (WebSocket-mocked). Both suites verified to actually catch a deliberately-broken assertion, not just pass vacuously. |
| 11. Deployment | **Prepared** (confirmed scope via AskUserQuestion: "prepare everything, you execute the account steps" — account/dashboard actions are inherently the user's, not mine). Target: Vercel + Railway, also confirmed via AskUserQuestion. Delivered: `backend-nest/Dockerfile.prod` (multi-stage, `node:22-slim`, `prisma migrate deploy` on startup) alongside the untouched dev `Dockerfile`; `backend-nest/.dockerignore`; `backend-nest/railway.toml`; `RedisService` now prefers a single `REDIS_URL` (Railway's managed-Redis shape) over `REDIS_HOST`/`REDIS_PORT`, falling back unchanged for local dev; `DEPLOYMENT.md` — the full ordered runbook (Railway plugins → backend service → real Google OAuth credentials → Vercel → close the CORS/`FRONTEND_URL` loop → smoke test), plus the complete environment-variable checklist per service. Actually creating accounts/deploying is the next step, on the user's side. |
