...
export default function BasketballGameDetail({ params }: { params: { id: string } }) {
  ...
  return (
    <ThemeLayout
      sport="basketball"
      breadcrumbExtra={
        data ? [{ label: `${data.game.home_team.name} vs ${data.game.away_team.name}`, href: `/basketball/games/${id}` }] : []
      }
    >
            ...
            <TabsContent value="overview" className="space-y-6">
              ...
              {keyPlayer && (
                <KeyPlayerCard
                  ...
                  href={`/basketball/players/${keyPlayer.id}`}
                />
              )}
