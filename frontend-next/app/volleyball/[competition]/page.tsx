...
export default function VolleyballCompetitionHub({ params }: { params: { competition: string } }) {
  ...
  useEffect(() => {
    fetchCompetitions("volleyball", lang).then((list) => setLabel(list.find((c) => c.slug === competition)?.name));
  }, [competition, lang]);
  useEffect(() => {
    fetchVolleyballGames(lang, competition).then(setGames);
  }, [lang, competition]);
  useEffect(() => {
    fetchStandings("volleyball", competition, lang).then(setStandings);
  }, [lang, competition]);
  useEffect(() => {
    fetchNews(30, "volleyball", competition).then(setNews);
  }, [lang, competition]);
  useEffect(() => {
    fetchTransfers(30, "volleyball", competition).then(setTransfers);
  }, [lang, competition]);
  ...
  return (
    <ThemeLayout
      ...
      breadcrumbExtra={label ? [{ label, href: `/volleyball/${competition}` }] : []}
    >
      <Tabs defaultValue="live" className="mt-2">
        ...
        <TabsContent value="live" className="space-y-3">
          ...
          {live.map((g) => (
            <GameCard key={g.id} game={g} sport="volleyball" t={t} />
          ))}
        </TabsContent>
