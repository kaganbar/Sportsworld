...
import {
  VolleyballGameDetail as VolleyballGameDetailData,
  FormStats,
  LineupEntry,
  PastResult,
...
export default function VolleyballGameDetail({ params }: { params: { id: string } }) {
  const { id } = params;
  const { t, lang } = useLang();
  const [data, setData] = useState<VolleyballGameDetailData | null>(null);
  const [error, setError] = useState<string | null>(null);

  useEffect(() => {
    fetchVolleyballGameDetail(id, lang).then(setData).catch((e) => setError(String(e)));
  }, [id, lang]);

  useLiveGame(
    data?.game.status === "live" ? `/ws/games/volleyball/${id}/` : null,
    (payload) => {
      setData((prev) => {
        if (!prev) return prev;
  ...
  return (
    <ThemeLayout
      sport="volleyball"
      breadcrumbExtra={
        data ? [{ label: `${data.game.home_team.name} vs ${data.game.away_team.name}`, href: `/volleyball/games/${id}` }] : []
      }
    >
            ...
            <TabsContent value="overview" className="space-y-6">
              ...
              {keyPlayer && (
                <KeyPlayerCard
                  ...
                  href={`/volleyball/players/${keyPlayer.id}`}
                />
              )}
                    ...
                    <table className="w-full text-sm" dir="ltr">
                      <thead>
                        <tr>
                          <th scope="col" />
                          {data.sets.map((s) => (
                            <th key={s.set_number} scope="col" className="px-2 py-1 text-center text-muted-foreground">{s.set_number}</th>
                          ))}
                        </tr>
                      </thead>
                      <tbody>
                        <tr>
                          ...
                          {data.sets.map((s) => (
                            <td key={s.set_number} className="px-2 py-1 text-center">{s.home_score}</td>
                          ))}
                        </tr>
                        <tr>
                          <th scope="row" className="pe-3 text-start font-medium">{data.game.away_team.short_name}</th>
                          {data.sets.map((s) => (
                            <td key={s.set_number} className="px-2 py-1 text-center">{s.away_score}</td>
                          ))}
                        </tr>
                      </tbody>
                    </table>
