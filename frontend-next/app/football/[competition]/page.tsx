...
export default function FootballCompetitionHub({ params }: { params: { competition: string } }) {
  ...
  useEffect(() => {
    fetchCompetitions("football", lang).then((list) => setLabel(list.find((c) => c.slug === competition)?.name));
  }, [competition, lang]);
  useEffect(() => {
    fetchTodaysGames(lang, competition).then(setGames);
  }, [lang, competition]);
  useEffect(() => {
    fetchStandings("football", competition, lang).then(setStandings);
  }, [lang, competition]);
  useEffect(() => {
    fetchNews(30, "football", competition).then(setNews);
  }, [lang, competition]);
  useEffect(() => {
    fetchTransfers(30, "football", competition).then(setTransfers);
  }, [lang, competition]);
  ...
  return (
    <ThemeLayout
      ...
      breadcrumbExtra={label ? [{ label, href: `/football/${competition}` }] : []}
    >
      <Tabs defaultValue="live" className="mt-2">
        ...
        <TabsContent value="live" className="space-y-3">
          ...
          {live.map((g) => (
            <GameCard key={g.id} game={g} sport="football" t={t} />
          ))}
        </TabsContent>
                ...
                <table className="w-full text-sm" dir="ltr">
                  <thead>
                    <tr className="text-white/60">
                      ...
                      <th scope="col" className="px-2 py-1">{t("drawsAbbr")}</th>
                      <th scope="col" className="px-2 py-1">{t("lossesAbbr")}</th>
                      <th scope="col" className="px-2 py-1">GD</th>
                  ...
                  <tbody>
                    {standings.map((row, i) => (
                      <tr key={row.team_id} className="border-t border-white/10">
                        ...
                        <td className="px-2 py-1 text-center">{row.goal_diff}</td>
