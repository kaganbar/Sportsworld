...
export default function BaseballCompetitionHub({ params }: { params: { competition: string } }) {
  ...
  useEffect(() => {
    fetchCompetitions("baseball", lang).then((list) => setLabel(list.find((c) => c.slug === competition)?.name));
  }, [competition, lang]);
  useEffect(() => {
    fetchBaseballGames(lang, competition).then(setGames);
  }, [lang, competition]);
  useEffect(() => {
    fetchStandings("baseball", competition, lang).then(setStandings);
  }, [lang, competition]);
  useEffect(() => {
    fetchNews(30, "baseball", competition).then(setNews);
  }, [lang, competition]);
  useEffect(() => {
    fetchTransfers(30, "baseball", competition).then(setTransfers);
  }, [lang, competition]);
  ...
  return (
    <ThemeLayout
      ...
      breadcrumbExtra={label ? [{ label, href: `/baseball/${competition}` }] : []}
    >
      <Tabs defaultValue="live" className="mt-2">
        ...
        <TabsContent value="live" className="space-y-3">
          ...
          {live.map((g) => (
            <GameCard key={g.id} game={g} sport="baseball" t={t} />
          ))}
        </TabsContent>
