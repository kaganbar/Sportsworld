...
import {
  BaseballGameDetail as BaseballGameDetailData,
  FormStats,
  LineupEntry,
  PastResult,
...
export default function BaseballGameDetail({ params }: { params: { id: string } }) {
  const { id } = params;
  const { t, lang } = useLang();
  const [data, setData] = useState<BaseballGameDetailData | null>(null);
  const [error, setError] = useState<string | null>(null);

  useEffect(() => {
    fetchBaseballGameDetail(id, lang).then(setData).catch((e) => setError(String(e)));
  }, [id, lang]);

  useLiveGame(
    data?.game.status === "live" ? `/ws/games/baseball/${id}/` : null,
    (payload) => {
      setData((prev) => {
        if (!prev) return prev;
  ...
  return (
    <ThemeLayout
      sport="baseball"
      breadcrumbExtra={
        data ? [{ label: `${data.game.home_team.name} vs ${data.game.away_team.name}`, href: `/baseball/games/${id}` }] : []
      }
    >
            ...
            <TabsContent value="overview" className="space-y-6">
              ...
              {keyPlayer && (
                <KeyPlayerCard
                  ...
                  href={`/baseball/players/${keyPlayer.id}`}
                />
              )}
